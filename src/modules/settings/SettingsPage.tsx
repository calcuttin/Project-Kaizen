...
export function SettingsPage() {
  ...
  const doExport = () => {
    ...
    a.href = URL.createObjectURL(blob); a.download = `kaizen-backup-${new Date().toISOString().slice(0, 10)}.json`; a.click();
    URL.revokeObjectURL(a.href);
    toast('Backup downloaded');
  };
  const doImport = async (file: File) => {
    try { importAll(JSON.parse(await file.text())); toast('Data restored'); }
  ...
  return (
    <div className="page">
      ...
      <div className="grid cols-2">
        ...
        <Card title="About" className="span-2">
          <p className="muted" style={{ fontSize: 13.5, lineHeight: 1.6 }}>
            ...
            Every area is a self-contained module, so new ones (finance, relationships, journaling, travel) can be added without touching the rest.
          </p>
        </Card>
      ...
      <Modal open={showConflicts} onClose={() => setShowConflicts(false)} title="Conflict inbox"><div className="stack" style={{ gap: 12 }}>{conflicts.map((conflict) => <div key={conflict.id} style={{ border: '1px solid var(--border)', borderRadius: 10, padding: 12 }}><div className="between"><strong style={{ fontSize: 13 }}>{conflict.entityType} · {conflict.entityId}</strong><span className="muted" style={{ fontSize: 10 }}>{new Date(conflict.detectedAt).toLocaleString()}</span></div><p className="muted" style={{ fontSize: 11, margin: '8px 0' }}>This item changed on two devices. Choose which complete version to keep.</p><div className="row" style={{ justifyContent: 'flex-end' }}><Button size="sm" onClick={() => void resolveConflict(conflict, 'remote')}>Use cloud version</Button><Button size="sm" variant="primary" onClick={() => void resolveConflict(conflict, 'local')}>Keep this device</Button></div></div>)}</div></Modal>
    </div>
  );
}
